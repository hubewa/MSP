...
void apply_filter(my_image_comp *in, my_image_comp *out, int threshold)
{
  ...
  main(int argc, char *argv[])
{
  if (argc != 4)
    {
      fprintf(stderr,"Usage: %s <in bmp file> <out bmp file> T\n",argv[0]);
      return -1;
    }
  ...
  try {
      ...
      for (n=0; n < num_comps; n++)
        apply_filter(input_comps+n,output_comps+n, threshold);
